...
def plot_revenue_per_sales_channel_pie_chart(df):
    # Generate the pie chart
    chart = alt.Chart(df).mark_arc(innerRadius=50).encode(
        alt.Theta('revenue:Q', title='Total Revenue', stack=True),
        alt.Color('sales_channel_id:N', title='Sales Channel'),
        tooltip=['sales_channel_id', 'revenue']
    ).properties(
        title='Total Revenue per Sales Channel',
        width=400,
        height=400
    )
...
# KPI
    ...
    revenue_per_sales_channel_pie_chart = plot_revenue_per_sales_channel_pie_chart(filter_saleschannel_df)
    st.altair_chart(revenue_per_sales_channel_pie_chart, use_container_width=True)
